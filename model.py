...
class Critic(object):
    def __init__(
        ...
        self.critic_optimizer = torch.optim.Adam(self.critic.paramers(), lr=3e-4)
        ...
        if self.policy_type == "Gaussian":
            ...
            self.policy = GaussianPolicy(state_dim, action_dim.shape[0], 26, action_dim).to(device)
            self.policy_optim = torch.optim.Adam(self.policy.parameters(), lr=3e-4)

        else:
            self.alpha = 0
            self.automatic_entropy_tuning = False
            self.policy = DeterministicPolicy(state_dim, action_dim.shape[0], 256, action_dim).to(self.device)
            self.policy_optim = torch.optim.Adam(self.policy.parameters(), lr=3e-4)
        ...
        for _ in range(nr_of_steps):
            ...
            with torch.no_grad():
                for idx, actor in enumerate(self.actors):
                    next_action, next_state_log_pi, _ = self.actor_targets[idx].sample(next_state)
